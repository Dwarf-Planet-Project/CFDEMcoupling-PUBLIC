...
) const
{
    ...
    // AYCOCK:
    // TODO: Write code to store variable in MEMORY every timestep so that it can be accessed by the force model:
    // SOLVED: see "http://www.cfd-online.com/Forums/openfoam-programming-development/88823-objectregistry-lookupobject-scalar-3.html#post471786"
    //         for explanation.
    IOdictionary& myGradPDict = const_cast<IOdictionary&>(mesh_.lookupObject<IOdictionary>("myGradP"));
    myGradPDict.set("gradient", gradP);
}
    ...
    invAPtr_(NULL)
{
    ...
    IFstream propsFile
    (
        mesh_.time().timePath()/(name_ + "Properties")
    );
