...
#include "ShirgaonkarIBpressure.H"
...
defineTypeNameAndDebug(ShirgaonkarIBpressure, 0);

addToRunTimeSelectionTable
(
    forceModel,
    ShirgaonkarIBpressure,
    dictionary
);
...
ShirgaonkarIBpressure::ShirgaonkarIBpressure
(
...
void ShirgaonkarIBpressure::setForce() const
{
    ...
    for(int index=0; index< particleCloud_.numberOfParticles(); index++)
    {
        ...
        for(int subCell=0;subCell<particleCloud_.voidFractionM().cellsPerParticle()[index][0];subCell++)
        {
            ...
            if (cellI > -1) // particle Found
            {
                ...
                //drag += h[cellI]*h.mesh().V()[cellI] * (1-voidFraction_[cellI]); // Is this more accurate?
            }
        ...
        if(Pstream::master())
        {
            ...
            forceFileOutput.open ("f_ShirgaonkarIBpressure.csv", std::ofstream::app);
