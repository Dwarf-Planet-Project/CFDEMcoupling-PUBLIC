    ...
    propsDict_(dict.subDict(typeName + "Props"))
{
    ...
    firstTimeStep_ = true; // AYCOCK
    secondTimeStep_ = false; // AYCOCK
    ...
    // If current processor is the master proc.:
    if (me == 0)
    {
...
bool Foam::twoWayMPI::couple() const
{
    ...
    //if (doCoupleNow() || secondTimeStep_)
    if (1) // Couple during every call to couple() (for tight coupling; AYCOCK)
    {
        couplingStep_++;
        coupleNow = true;
        ...
        if (liggghts == 1)
        {
            ...
            // check if liggghtsCommandModels with exact timing are being run
            ...
            if(exactTiming)
            {
                ...
                if(particleCloud_.liggghtsCommand()[runComNr]().runCommand(couplingStep()))
                {
                    ...
                    const char* command = particleCloud_.liggghtsCommand()[runComNr]().command(0);
                    Info << "364 Executing run command: '"<< command <<"'"<< endl;
                    lmp->input->one(command);
                }
                ...
                forAll(particleCloud_.liggghtsCommandModelList(),i)
                {
                    ...
                    )
                    {
                        ...
                        for(int j=0;j<commandLines;j++)
                        {
                            ...
                            Info << "381 Executing command: '"<< command <<"'"<< endl;
                            lmp->input->one(command);
                        }
                    }
                }
                ...
                forAll(particleCloud_.liggghtsCommandModelList(),i)
                {
                    if(particleCloud_.liggghtsCommand()[i]().runCommand(couplingStep()))
                    {
                        ...
                        for(int j=0;j<commandLines;j++)
                        {
                            ...
                            if(firstTimeStep_) {
                                Info << "*************************" << endl;
                                Info << "INITIALIZING WITH 'run 0'" << endl;
                                Info << "*************************" << endl;
                                lmp->input->one("run 0");
                                firstTimeStep_ = false;
                                secondTimeStep_ = true;
                            }
                            else {
                                Info << "398 Executing command: '"<< command <<"'"<< endl;
                                lmp->input->one(command);
                            }
                        }
                    }
