...
void IBVoidFraction::setvoidFraction(double** const& mask,double**& voidfractions,double**& particleWeights,double**& particleVolumes) const
{
    ...
    for(int index=0; index< particleCloud_.numberOfParticles(); index++)
    {
        ...
        //{
            ...
            // particleV[index][0]=0; // Added in newest version of CFDEM; was this a bug? 

            //collecting data
            label particleCenterCellID=particleCloud_.cellIDs()[index][0];
            label particleCenterCellID2=particleCloud_.cellIDs()[index][0];
            //Pout << "particleCenterCellID: " << particleCenterCellID2 << endl;
            reduce(particleCenterCellID2, sumOp<label>());
            ...
            if (particleCenterCellID >= 0)
            {
                ...
                else
                {
                    ...
                    forAll(vertices, i)
                    {
                        ...
                        if(centreDist<=radius) // Aycock
                        //if(centreDist<radius &&  centreVertexDist<radius)
                        {
                            if(centreVertexDist<=radius) voidfractionNext_[particleCenterCellID]-=0.125;
                            else //(centreVertexDist>=radius) // Aycock
                            {
                                //compute lambda
                                ...
                                //Pout << "Case 1, Lambda: " << lambda << endl;
                            }
                        }
                        else if(centreVertexDist<=radius) // Aycock
                        //else if(centreDist>radius && centreVertexDist<radius) 
                        {
                            ...
                            scalar b = 2.0 * (vertexPosition - cellCentrePosition) & (cellCentrePosition-minPeriodicParticlePos);
                            scalar c = ((cellCentrePosition-minPeriodicParticlePos) & (cellCentrePosition-minPeriodicParticlePos)) - radius*radius;
                            scalar lambda = 0.;
                            
                            if(b*b-4*a*c>=0)  lambda =  (-b+sqrt(b*b-4*a*c))/(2*a);
                            // Key difference: -=(1-lambda)*0.125
                            ...
                            //Pout << "Case 2, Lambda: " << lambda << endl;
                        }
                    }
      		...
      		if(checkPeriodicCells_) 
                ...
                {
                    FatalError<< "IB particle algo found more cells ("<< hashSetLength 
    ...
    forAll(nc,i){
        ...
        else if(!hashSett.found(neighbor) && centreDist < radius + sqrt(3.0)*pow(particleCloud_.mesh().V()[neighbor],0.33333))
        {
            ...
            forAll(vertexPoints, j)
            {
                ...
                {
                // ******************************************************* 
                // ^ This was the problem! But why does this matter? Doesn't the 'else if' part still work alright?
                // ******************************************************* 
                //if (centreDist < radius){
                    
                    // If the vertex AND the cell center are inside the particle:
                    if (vertexDist <= radius) scale -= 0.125;
                    // If the cell center is inside the particle, but the current vertex is outside of the particle:
                    else 
                    {
                        scalar a = (vertexPosition - cellCentrePosition)&(vertexPosition - cellCentrePosition);
                        ...
                        //Pout << " Cell ID " << neighbor <<  "; Case 3, Lambda: " << lambda << endl;
                    }
                }
                // If the current vertex is inside of the particle, but the cell center is outside of the particle:
                else if (vertexDist <= radius)
                {
                    scalar a = (vertexPosition - cellCentrePosition)&(vertexPosition - cellCentrePosition);
                    scalar b = 2.0 * (vertexPosition - cellCentrePosition)&(cellCentrePosition-position);
                    scalar c = ((cellCentrePosition-position)&(cellCentrePosition-position))-radius*radius;
                    scalar lambda = 0.0;
                    ...
                    //Pout << " Cell ID " << neighbor <<  "; Case 4, Lambda: " << lambda << endl;
                }
            }
